...
mod5 <- CAST::ffs(predictors = pred, response = res, method = "rf", importance = TRUE, trControl = trainctl, 
                  metric = "Kappa")
saveRDS(mod5, "J:/mod5.rds")
print(Sys.time())
gc()

mod5 <- readRDS(paste0(envrmt$path_data_training, "mod5.rds"))

test <- predict(mod5, traindat_all_2[smp,-9])
